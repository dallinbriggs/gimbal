...
class Geopointer(object):
    ...
    def __init__(self):
        ...
        self.gimbal_pub_log = rospy.Publisher("gimbal/log", Gimbal_log, queue_size=1)
    ...
    def compute_gimbal_control(self):
        ...
        gimb_log = Gimbal_log()
        gimb_log.header.stamp = rospy.Time.now()
        gimb_log.gimbal_vector = vec
        gimb_log.att_vector = Vector3(self.phi, self.theta, self.psi)
        gimb_log.pos_vector = Vector3(self.pos[0], self.pos[1], self.pos[2])
        self.gimbal_pub_log.publish(gimb_log)
...
def main():
    ...
    rospy.init_node("gimbal_logger", log_level=rospy.DEBUG)
